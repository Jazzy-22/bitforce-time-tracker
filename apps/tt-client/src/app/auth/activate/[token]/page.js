...
export default function Activation({ params }) {
    ...
    useEffect(() => {
        if (!firstLoad) return;
        axios.post(`${process.env.NEXT_PUBLIC_API_URL}/auth/activate/${token}/${email}`)
        .then(res => {
            setFirstResponse({title: 'Your account has been activated!', message: 'Please enter a new password.'});
            setRecoveryToken(res.data.recovery_token);
        })
        .catch(err => {
            setFirstResponse({title: 'We could not activate your account.', message: err.response.data.message});
        })
        .finally(() => {
            setIsReady(true);
        });
